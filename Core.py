...
import requests
from Functions import FindUserId,UserGameBan, GetGameBanHistory,UnGameBan,GetPlayerHistory
...
class Core(commands.Cog):
  ...
  @app_commands.command(name="get-gamebanhistory", description= "Get full banlogs of people who got banned via ban api.")
  # @discord.app_commands.checks.has_any_role("Game Moderator","Admin", "Head Admin", "Creator")
  # Test server role id
  @discord.app_commands.checks.has_role(1384526591173853316)
  @app_commands.checks.cooldown(2, 60.0, key=lambda i: i.user.id)
  async def GameBanHistory(self, interaction: discord.Interaction):
  ...
  @discord.app_commands.checks.has_role(1384526591173853316)
  @app_commands.checks.cooldown(1, 25.0, key=lambda i: i.user.id)
  async def UnGameban(self, interaction: discord.Interaction,player: str,display_reason: str,private_reason: str = "No private reason provided!"):
    await interaction.response.defer(thinking=True)
    userId = FindUserId(player)
    ...
    try:
      if isinstance(error, app_commands.CommandOnCooldown):
        ...
        await interaction.response.send_message(f"An error occurred: {error}", ephemeral=True)
            ...
            print(f"Network error while handling command error: {e}")
  @app_commands.command(name="getplayerbans", description="Get players ban history"):
  #@discord.app_commands.checks.has_any_role("Game Moderator","Admin", "Head Admin", "Creator")
  @discord.app_commands.checks.has_role(1384526591173853316)
  @app_commands.checks.cooldown(1, 25.0, key=lambda i: i.user.id)
  async def GetPlayerBanHistory(self, interaction: discord.Interaction, player: str):
    userId = FindUserId(player)
    PlayerHistory = GetPlayerHistory(userId, player)
    interaction.response.send_message(PlayerHistory)
